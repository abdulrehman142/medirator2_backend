"""Grok (xAI) generation with structured clinical prompts."""
...
# Prefer GROK_* ; accept XAI_* as aliases used in xAI docs
GROK_API_KEY = (
...
def grok_reachable() -> bool:
    """True if API key is set and /models responds OK."""
    if not GROK_API_KEY:
        return False
    try:
...
def resolve_model() -> str:
    ...
    # Use configured model; optionally verify it exists in /models
    models = list_models()
    if models and GROK_MODEL not in models:
        # Prefer exact match; otherwise keep configured (aliases may not list)
        for name in models:
            if name.startswith(GROK_MODEL) or GROK_MODEL.startswith(name):
                return name
    return GROK_MODEL
...
def generate(
    ...
    """Generate answer via Grok (xAI); fall back to deterministic structured output."""
    ...
    if not grok_configured():
        return (
            fallback.get(
                "summary",
                "Grok API key not configured; showing retrieved record.",
            ),
            fallback,
            "fallback-no-key",
        )
